...
if __name__ == '__main__':
    ...
    color_calib_file = 'color/data/color_calib.json'
    ...
    volume_imgs = torch.tensor(volume_imgs, device='cuda')
    ...
    for start_idx in tqdm(range(0, num_images, batch_size)):
        ...
        blend_volume_imgs = blend_volume_imgs[k_z//2:blend_volume_imgs.shape[0]-k_z//2, :, :, :]  # 去除padding部分
        blend_volume_imgs = blend_volume_imgs[:, :blend_volume_imgs.shape[1]-edge_adjust[1], :blend_volume_imgs.shape[2]-edge_adjust[2], :]
        ...
        blend_volume_imgs[:, :, :, 0:3] = (blend_volume_imgs[:, :, :, 0:3] * blend_volume_imgs[:, :, :, [3]] + volume_color[k_z//2:blend_volume_imgs.shape[0]+k_z//2, :, :] * volume_density_cliped[k_z//2:blend_volume_imgs.shape[0]+k_z//2, :, :].unsqueeze(-1)) / (blend_volume_imgs[:, :, :, [3]] + volume_density_cliped[k_z//2:blend_volume_imgs.shape[0]+k_z//2, :, :].unsqueeze(-1) + 1e-9)
        blend_volume_imgs[:, :, :, 3] += volume_density_cliped[k_z//2:blend_volume_imgs.shape[0]+k_z//2, :, :]
    ...
    volume_imgs[:, :, :, 3] -= 1 # magic_num

    print("Calculating Images...")

    
    
    def process_image(i, path,img):
        # 计算浓度图像
        concentration_img = calculate_concentration(img)
        # 转换浓度图像为颜色
    ...
    Parallel(n_jobs=-1)(delayed(process_image)(i, image_paths[i], volume_imgs[i]) for i in range(len(volume_imgs)))
